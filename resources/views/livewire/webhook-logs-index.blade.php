<div>
    <x-mary-header
        title="{{ $userId ? 'Logs do Usuário' : 'Logs de Webhooks' }}"
        subtitle="Visualize os logs gerados pelos webhooks."
        separator>
        <x-slot:middle class="!justify-end">
            <x-mary-input icon="o-magnifying-glass" wire:model.live="search" placeholder="Pesquisar..." />
        </x-slot:middle>
        <x-slot:actions>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        @foreach($webhookTypeOptions as $option)
            <x-mary-card
                ...
                separator
                wire:click="selectWebhookType('{{ $option['id'] }}')"
                class="{{ $webhookTypeFilter == $option['id'] ? 'border-2 border-primary' : '' }}">
                <x-mary-button label="Selecionar" wire:click="selectWebhookType('{{ $option['id'] }}')" />
            </x-mary-card>
        @endforeach
    </div>
    ...
    <x-mary-drawer
        ...
        subtitle="Ajuste os filtros para personalizar a visualização dos logs."
        separator
        with-close-button
        close-on-escape
